...
public class ProjectDetails extends HttpServlet {
	...
	Double prn;
	String ptitle,qualification,description,platform,duration;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		//GET ALL PARAMETERS
		ptitle=request.getParameter("project_title");
		qualification=request.getParameter("qualifiacation");
		platform=request.getParameter("project_platform");
		description=request.getParameter("project_description");
		duration=request.getParameter("project_duration");
		
		//TO CREATE PRINTWRITER OBJECT
		/*PrintWriter out=response.getWriter();
		
		out.println(prn+""+ptitle+" "+platform+" "+qualification+" "+description+" "+duration);
		*/
		...
		try 
		{
			ps=con.prepareStatement("insert into project_details values (?,?,?,?,?,?)");
