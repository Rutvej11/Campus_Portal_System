...
@WebServlet("/CheckViewList")
public class CheckViewList extends HttpServlet {
		...
		try 
		{
			...
			if(rs.first())
			{
				...
				if(rs.getString(1).equals("Yes"))
				{
					...
					rd= request.getRequestDispatcher("/View_StudentList_Company.jsp");
					rd.forward(request, response);
				}
